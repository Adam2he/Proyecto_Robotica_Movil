...
def planificador(ini, goal, m, obstacles, m_goal, R_soi, R_obstacle, R_robot, N, maximum_step_size, path_pub):
    steps=np.array([ini])
...
if __name__ == "__main__":
    try:
        ...
        if true_dimension:
            robot_scale = [R_robot*2, R_robot*2, R_robot]
        else:
            robot_scale = [1, 1, 0.5]
        ...
        publish_obstacles(obstacles_pub, obstacles, r=255, g=128, b=0)
        ...
        while not origen_valido:
            ...
            for obstacle in obstacles:
                if calcula_distancia(origen, obstacle) <= R_obstacle+R_robot:
                    origen_valido = False
        publish_marker(marker_pub, marker_id=1, position = origen, r=1.0, g=1.0, b=0.0, scale=robot_scale, marker_type = Marker.CUBE)
        ...
        while not destino_valido:
            ...
            for obstacle in obstacles:
                if calcula_distancia(destino, obstacle) <= R_obstacle+R_robot:
                    destino_valido = False
        publish_marker(marker_pub, marker_id=2, position = destino, r=1.0, g=1.0, b=1.0, scale=robot_scale, marker_type = Marker.CUBE)
        
        #steps
        steps, status = planificador(origen,destino,m=m,obstacles=obstacles,m_goal=m_goal,R_soi=R_soi,R_obstacle=R_obstacle, R_robot=R_robot,N=N,maximum_step_size=maximum_step_size,path_pub=path_pub)
        #publish_path(path_pub, steps)
        
        #mover el robot
        for i in range(len(steps)-1):
            intra_steps=np.linspace(steps[i], steps[i+1], 2)[1:]
            for intra_step in intra_steps:
                publish_marker(marker_pub, marker_id=1, position = intra_step, r=1.0, g=1.0, b=0.0, scale=robot_scale, marker_type = Marker.CUBE)
        if status == "success":
                publish_marker(marker_pub, marker_id=2, position = destino, r=1.0, g=1.0, b=1.0, scale=[0, 0, 0], marker_type = Marker.CUBE)
                publish_marker(marker_pub, marker_id=1, position = steps[-1], r=0.0, g=1.0, b=0.0, scale=robot_scale, marker_type = Marker.CUBE)
        elif status == "fail_collision":
                publish_marker(marker_pub, marker_id=1, position = steps[-1], r=1.0, g=0.0, b=0.0, scale=robot_scale, marker_type = Marker.CUBE)
